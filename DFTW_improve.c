...
int DFT(int idft, double *xr, double *xi, double *Xr_o, double *Xi_o, int N, double *Xr_n_o, double *Xi_n_o);
...
int main(int argc, char *argv[])
{
    ...
    // Allocate array for each thread
    double Xr_n_o[MAX_THREADS];
    double Xi_n_o[MAX_THREADS];
    ...
    // DFT
    int idft = 1;
    DFT(idft, xr, xi, Xr_o, Xi_o, N, Xr_n_o, Xi_n_o);
    // IDFT
    idft = -1;
    DFT(idft, Xr_o, Xi_o, xr_check, xi_check, N, Xr_n_o, Xi_n_o);
...
#pragma omp parallel
        {
            ...
            Xr_n_o[id] = 0;
            Xi_n_o[id] = 0;
            for (int n = id; n < N; n += numthreads)
            {
                Xr_n_o[id] += xr[n] * cos(n * k * PI2 / N) + idft * xi[n] * sin(n * k * PI2 / N);
                Xi_n_o[id] += -idft * xr[n] * sin(n * k * PI2 / N) + xi[n] * cos(n * k * PI2 / N);
            }
        }
